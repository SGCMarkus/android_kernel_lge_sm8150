...
int    broadcast_fc8300_drv_if_power_on(void)
{
    ...
    if (!isdbt_fc8300_is_power_on())
        rc = isdbt_fc8300_power_on();
    return rc;
}
...
int    broadcast_fc8300_drv_if_get_dmb_data(struct broadcast_dmb_data_info *pdmb_data)
{
    ...
    total_len = isdbt_fc8300_get_ts((void*)((unsigned long)pdmb_data->data_buf_addr), pdmb_data->data_buf_size);
