        ...
        enum BROADCAST_TYPE broadcast)
{
    ...
    if ((broadcast_type == ISDBT_13SEG) || (broadcast_type == ISDBT_1SEG) ||
                            (catv_status == 2)) {
        fc8300_set_freq(handle, devid, 545143);
...
s32 fc8300_set_freq(HANDLE handle, DEVICEID devid, u32 freq)
{
    ...
    case ISDBT_CATV_13SEG:
        if (freq < 273143) {
            if (catv_status != 1) {
                catv_status = 1;
                fc8300_tuner_init(handle, devid,
                        broadcast_type);
            }
...
#ifdef BBM_EXT_LNA
    fc8300_write(handle, devid, 0xe0, 0x03);
    fc8300_write(handle, devid, 0xe5, 0xb9);    /*on value = 255 + (wanted value)*/ /*current (-70)*/
    fc8300_write(handle, devid, 0xe6, 0xd7);    /*off value = 255 + (wanted value)*/ /*current (-40)*/
    fc8300_write(handle, devid, 0xe8, 0x14);
    fc8300_write(handle, devid, 0xe9, 0x81);
#endif
    return BBM_OK;
}
