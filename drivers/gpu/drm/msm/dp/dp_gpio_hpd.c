...
static irqreturn_t dp_gpio_isr(int unused, void *data)
{
...
#if IS_ENABLED(CONFIG_LGE_COVER_DISPLAY)
	pr_info("%s : DD Before Debounce Time : %s", __func__,
		hpd ? "CONNECT" : "DISCONECT");

	if (lge_dp->led_status) {
		pr_info("%s : skip DD HPD low operation for cover_led\n", __func__);
		return IRQ_HANDLED;
	}
	...
	if (!hpd) {
		...
		if (state && is_dd_powermode()) {
			gpio_hpd->base.need_to_recovery = 1;
			pr_info("%s : need_to_reset = %d\n", __func__, gpio_hpd->base.need_to_recovery);
		}
	}

	if (hpd && gpio_hpd->base.need_to_skip_real_disconnect) {
		gpio_hpd->base.need_to_skip_real_disconnect = 0;
		complete_all(&lge_dp->recovery_off_comp);
		pr_info("%s : need_to_skip_real_disconnect\n", __func__);
	}

	if (hpd && gpio_hpd->base.need_to_recovery) {
		struct lge_cover_ops *ops = get_lge_cover_ops();
		pr_info("%s : need_to_DD_recovery\n", __func__);

		gpio_hpd->base.need_to_recovery = 0;
		complete_all(&lge_dp->recovery_off_comp);
		...
		if (gpio_hpd->base.recovery_count++ < MAX_RECOVERY_COUNT) {
			...
			if (ops)
				ops->set_recovery_state(RECOVERY_POWERDROP_BEGIN);
