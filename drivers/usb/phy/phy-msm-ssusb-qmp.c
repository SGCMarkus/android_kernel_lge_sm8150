...
#define INIT_MAX_TIME_USEC			2000
#else
...
static void usb_qmp_update_portselect_phymode(struct msm_ssphy_qmp *phy)
{
	...
	case USB_PHY_TYPE_USB3_AND_DP:
		...
		if (!(phy->phy.flags & PHY_USB_DP_CONCURRENT_MODE)) {
			if (!is_dd_working()) {
				dev_info(phy->phy.dev, "reset USB3PHY and DPPHY(DD=%d, DD recovery=%d DD wait disconnect=%d)\n",
						is_dd_connected(),
...
#ifdef CONFIG_LGE_USB_GADGET
		dev_err(uphy->dev, "timeout rest:%u\n");
#endif
...
#if IS_ENABLED(CONFIG_LGE_COVER_DISPLAY)
	if (is_dd_working()) {
		dev_info(uphy->dev, "%s: DD is working, skip PHY reset\n", __func__);
		goto exit;
	}
...
#if IS_ENABLED(CONFIG_LGE_COVER_DISPLAY)
	if (is_dd_working()) {
		dev_info(uphy->dev, "%s: DD is working, ignore ssphy %s\n", __func__,
				(suspend ? "suspend" : "resume"));
		return 0;
	}
#endif
...
#if IS_ENABLED(CONFIG_LGE_COVER_DISPLAY)
	if (is_dd_working()) {
		dev_info(uphy->dev, "%s: DD is working, ignore notify\n", __func__);
		phy->cable_connected = true;
		return 0;
	}
	dev_info(uphy->dev, "QMP phy connect notification(is_dd_working=%d)\n", is_dd_working());
#else
				       ...
				       enum usb_device_speed speed)
{
	...
	hrtimer_cancel(&phy->timer);
#if IS_ENABLED(CONFIG_LGE_COVER_DISPLAY)
	dev_info(uphy->dev, "QMP phy disconnect notification(is_dd_working=%d)\n", is_dd_working());
#else
