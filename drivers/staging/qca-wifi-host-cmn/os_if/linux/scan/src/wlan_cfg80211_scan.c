...
void wlan_scan_acquire_wake_lock_timeout(struct wlan_objmgr_psoc *psoc,
					 qdf_wake_lock_t *scan_wk_lock,
					 uint32_t timeout)
{
	if (!psoc || !scan_wk_lock)
		return;

	if (ucfg_scan_wake_lock_in_user_scan(psoc))
		qdf_wake_lock_timeout_acquire(scan_wk_lock, timeout);
}
				 ...
				 qdf_wake_lock_t *scan_wk_lock)
{
	...
	if (ucfg_scan_wake_lock_in_user_scan(psoc))
		qdf_wake_lock_release(scan_wk_lock,
