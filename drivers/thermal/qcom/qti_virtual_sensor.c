...
static const struct virtual_sensor_data lge_virtual_sensors_flash[] = {
	...
	/* 0.77*skin(quiet) + 7.13 : for tracking mmw0 vts */
	{
...
static const struct virtual_sensor_data lge_virtual_sensors_beta[] = {
	/* (-0.187)*xo + 1.384*skin + (-7.601) */
	{
		.virt_zone_name = "vts-virt-therm",
		.num_sensors = 2,
		.sensor_names = {"xo-therm", "skin-therm"},
		.coefficient_ct = 2,
		.coefficients = {-187, 1384},
		.avg_offset = -7601000,
		.avg_denominator = 1000,
		.logic = VIRT_WEIGHTED_AVG,
	},
...
#else
static const struct virtual_sensor_data lge_virtual_sensors_default[] = {
	{
		.virt_zone_name = "vts-virt-therm",
		.num_sensors = 2,
		.sensor_names = {"xo-therm", "skin-therm"},
...
#elif defined(CONFIG_MACH_SM8150_FLASH)
	vts_cnt = ARRAY_SIZE(lge_virtual_sensors_flash);
	for (; vts_idx < vts_cnt; vts_idx++) {
		tz = devm_thermal_of_virtual_sensor_register(dev,
				&lge_virtual_sensors_flash[vts_idx]);
	}
#elif defined(CONFIG_MACH_SM8150_BETA)
	tz = devm_thermal_of_virtual_sensor_register(dev,
			&lge_virtual_sensors_beta[0]);
#else
	tz = devm_thermal_of_virtual_sensor_register(dev,
			&lge_virtual_sensors_default[0]);
#endif
