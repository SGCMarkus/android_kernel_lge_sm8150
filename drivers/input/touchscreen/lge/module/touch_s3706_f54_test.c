...
const int DefaultTimeout_switch_page = 5;	/* In counts */
...
static bool switchPage(struct device *dev, int page)
{
	...
	do {
		...
		count++;
	} while ((int)data != page && (count < DefaultTimeout_switch_page));

	if (count >= DefaultTimeout_switch_page) {
...
static int CompareTRexShortTestReport(struct device *dev)
{
	...
	f54len += snprintf(f54buf + f54len, sizeof(f54buf) - f54len,
				"After Compare with Physical bit, TRex Short Test Data = ");
...
static void RunQueries(struct device *dev)
{
	...
	do {
		ret = Read8BitRegisters(dev, cAddr, &cFunc, 1);
		if (ret < 0) {
			TOUCH_I("%s: retry : %d\n", __func__, retry);
			if (retry++ < 2)
				continue;
