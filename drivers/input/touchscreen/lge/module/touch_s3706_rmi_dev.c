 ...
 * DOLLARS.
 */
#define TS_MODULE "[s3706_rmi_dev]"
...
#define CHAR_DEVICE_NAME "rmi_module"
#define DEVICE_CLASS_NAME "rmidev_module"
#define SYSFS_FOLDER_NAME "rmidev_module"
#define DEV_NUMBER 1
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	/* page read */
	ret = s3706_read(dev, PAGE_SELECT_REG, &page_old, sizeof(page_old));
	...
	if (page_old == page_new) {
		...
		ret = s3706_write(dev, PAGE_SELECT_REG,
	...
	/* read register */
	ret = s3706_read(dev, (u8)(addr & ~(MASK_8BIT << 8)), data, length);
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	/* page read */
	ret = s3706_read(dev, PAGE_SELECT_REG,
				&page_old, sizeof(page_old));
	...
	if (page_old == page_new) {
		...
		ret = s3706_write(dev, PAGE_SELECT_REG, &page_new, sizeof(page_new));
	...
	/* write register */
	ret = s3706_write(dev, (u8)(addr & ~(MASK_8BIT << 8)), data, length);
...
static irqreturn_t rmidev_sysfs_irq(int irq, void *dev_id)
{
	...
	sysfs_notify(&md->input->dev.kobj,
		...
		bool enable)
{
	...
	if (enable) {
		...
		retval = request_threaded_irq(md->irq, NULL,
				rmidev_sysfs_irq, irq_flags,
				PLATFORM_DRIVER_NAME, md);
		if (retval < 0) {
...
static ssize_t rmidev_sysfs_open_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct module_data *md = to_module(rmidev->ts_dev);
	unsigned int input;

	if (kstrtou32(buf, 10, &input) < 0) {
		TOUCH_I("%s : kstrtou32 error\n", __func__);
		return -EINVAL;
	}
	...
	*/
	...
	TOUCH_I("%s: module_interrupt_control error!\n", __func__);
	free_irq(md->irq, md);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	request_threaded_irq(md->irq, module_irq_handler,
		...
		struct device_attribute *attr, char *buf)
{
	...
	attn_state = gpio_get_value(md->dts.int_pin);
...
static struct s3706_rmidev_exp_fn rmidev_module = {
...
{
	s3706_rmidev_function(&rmidev_module, true);
...
{
	s3706_rmidev_function(&rmidev_module, false);
