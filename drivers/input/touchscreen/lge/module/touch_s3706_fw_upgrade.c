...
static int synaptics_rmi4_reg_read(struct device *dev,
		unsigned short addr, unsigned char *data, unsigned int len);

static int synaptics_rmi4_reg_write(struct device *dev,
		unsigned short addr, unsigned char *data, unsigned int len);
...
static void synaptics_rmi4_fwu_reset(struct device *ts_dev);
...
static int fwu_f51_force_data_init(void)
{
	...
	retval = synaptics_rmi4_reg_read(fwu->ts_dev,
			rmi4_data->f51_query_base_addr + 7,
			offset,
			sizeof(offset));
	if (retval < 0) {
		TOUCH_E("%s: Failed to read force data offset\n", __func__);
		return retval;
	}
	...
	retval = synaptics_rmi4_reg_read(fwu->ts_dev,
			...
			sizeof(query_count));
	if (retval < 0) {
		TOUCH_E("%s: Failed to read number of F51 query registers\n",
				__func__);
		return retval;
	}
	...
	if (query_count >= 10) {
		retval = synaptics_rmi4_reg_read(fwu->ts_dev,
				...
				sizeof(packet_info));
		if (retval < 0) {
			TOUCH_E("%s: Failed to read F51 packet register info\n",
					__func__);
			return retval;
		}
	...
	if (fwu->cal_data_size > fwu->cal_data_buf_size) {
		...
		if (!fwu->cal_data) {
			TOUCH_E("%s: Failed to alloc mem for fwu->cal_data\n",
					__func__);
...
static int fwu_allocate_read_config_buf(unsigned int count)
{
	if (count > fwu->read_config_buf_size) {
		...
		if (!fwu->read_config_buf) {
			TOUCH_E("%s: Failed to alloc mem for fwu->read_config_buf\n",
					__func__);
		...
		struct block_count *blkcount, struct physical_address *phyaddr)
{
	...
	if (partition_table == NULL) {
		TOUCH_E("%s: partition table is Null!!", __func__);
		return -EINVAL;
	}
	...
	for (ii = 0; ii < fwu->partitions; ii++) {
		...
		TOUCH_D(FW_UPGRADE, "%s: Partition entry %d:\n", __func__, ii);
		for (offset = 0; offset < 8; offset++) {
			TOUCH_D(FW_UPGRADE, "%s: 0x%02x\n",
		...
		case CORE_CODE_PARTITION:
			...
			TOUCH_I("%s: Core code block count: %d\n",
		...
		case CORE_CONFIG_PARTITION:
			...
			TOUCH_I("%s: Core config block count: %d\n",
		...
		case BOOTLOADER_PARTITION:
			...
			TOUCH_I("%s: Bootloader block count: %d\n",
		...
		case UTILITY_PARAMETER_PARTITION:
			...
			TOUCH_I("%s: Utility parameter block count: %d\n",
		...
		case DISPLAY_CONFIG_PARTITION:
			...
			TOUCH_I("%s: Display config block count: %d\n",
		...
		case FLASH_CONFIG_PARTITION:
			...
			TOUCH_I("%s: Flash config block count: %d\n",
		...
		case GUEST_CODE_PARTITION:
			...
			TOUCH_I("%s: Guest code block count: %d\n",
		...
		case GUEST_SERIALIZATION_PARTITION:
			...
			TOUCH_I("%s: Guest serialization block count: %d\n",
		...
		case GLOBAL_PARAMETERS_PARTITION:
			...
			TOUCH_I("%s: Global parameters block count: %d\n",
		...
		case DEVICE_CONFIG_PARTITION:
			...
			TOUCH_I("%s: Device config block count: %d\n",
...
static void fwu_parse_image_header_05_06(void)
{
	...
	if (fwu->img.contains_disp_config) {
		...
		retval = secure_memcpy(fwu->img.cstmr_product_id,
				...
				PRODUCT_ID_SIZE);
		if (retval < 0) {
			TOUCH_E("%s: Failed to copy custom product ID string\n",
					__func__);
		}
	...
	retval = secure_memcpy(fwu->img.product_id,
			...
			PRODUCT_ID_SIZE);
	if (retval < 0)
		TOUCH_E("%s: Failed to copy product ID string\n", __func__);
...
static int fwu_parse_image_info(void)
{
	...
	if (fwu->bl_version == BL_V7 || fwu->bl_version == BL_V8) {
		if (!fwu->img.contains_flash_config) {
			TOUCH_E("%s: No flash config found in firmware image\n",
					__func__);
			return -EINVAL;
		}
		...
		retval = fwu_parse_partition_table(fwu->img.fl_config.data,
				&fwu->img.blkcount, &fwu->img.phyaddr);
		if (retval < 0) {
			TOUCH_E("%s: failed to parse partition table\n", __func__);
			return retval;
		}
...
static int fwu_read_flash_status(void)
{
	...
	if (fwu->flash_status != 0x00) {
		TOUCH_E("%s: Flash status = %d, command = 0x%02x\n",
...
static int fwu_wait_for_idle(int timeout_ms, bool poll)
{
	...
	do {
		...
		count++;
		if (poll_new || (count == timeout_count)) {
...
static int fwu_write_f34_v7_command_single_transaction(unsigned char cmd)
{
	...
	retval = synaptics_rmi4_reg_write(fwu->ts_dev,
			data_base + fwu->off.partition_id,
			data_1_5.data,
			sizeof(data_1_5.data));
	if (retval < 0) {
		TOUCH_E("%s: Failed to write single transaction command\n",
				__func__);
		return retval;
	}
...
static int fwu_write_f34_v7_command(unsigned char cmd)
{
	...
	retval = synaptics_rmi4_reg_write(fwu->ts_dev,
			...
			sizeof(command));
	if (retval < 0) {
		TOUCH_E("%s: Failed to write flash command\n", __func__);
		return retval;
	}
...
static int fwu_write_f34_v7_partition_id(unsigned char cmd)
{
	...
	retval = synaptics_rmi4_reg_write(fwu->ts_dev,
			...
			sizeof(partition));
	if (retval < 0) {
		TOUCH_E("%s: Failed to write partition ID\n", __func__);
		return retval;
	}
...
static int fwu_read_f34_v7_partition_table(unsigned char *partition_table)
{
	...
	retval = synaptics_rmi4_reg_write(fwu->ts_dev,
			...
			sizeof(length));
	if (retval < 0) {
		TOUCH_E("%s: Failed to write transfer length\n", __func__);
		return retval;
	}

	retval = fwu_write_f34_command(CMD_READ_CONFIG);
	if (retval < 0) {
		TOUCH_E("%s: Failed to write command\n", __func__);
		return retval;
	}

	module_msleep(READ_CONFIG_WAIT_MS);

	retval = fwu_wait_for_idle(WRITE_WAIT_MS, true);
	if (retval < 0) {
		TOUCH_E("%s: Failed to wait for idle status\n", __func__);
		return retval;
	}
	...
	if (retval < 0) {
		TOUCH_E("%s: Failed to read block data\n", __func__);
		return retval;
	}
...
static int fwu_read_f34_v7_queries(void)
{
	...
	retval = synaptics_rmi4_reg_read(fwu->ts_dev,
			...
			sizeof(query_0.data));
	if (retval < 0) {
		TOUCH_E("%s: Failed to read query 0\n", __func__);
		return retval;
	}
	...
	if (retval < 0) {
		TOUCH_E("%s: Failed to read queries 1 to 7\n", __func__);
		return retval;
	}
	...
	if (!ptable) {
		TOUCH_E("%s: Failed to alloc mem for partition table\n",
				__func__);
		return -ENOMEM;
	}
	...
	if (retval < 0) {
		TOUCH_E("%s: Failed to read partition table\n", __func__);
		kfree(ptable);
		return retval;
	}

	retval = fwu_parse_partition_table(ptable, &fwu->blkcount, &fwu->phyaddr);
	if (retval < 0) {
		TOUCH_E("%s: failed to parse partition table\n", __func__);
		kfree(ptable);
		return retval;
	...
	if (fwu->flash_properties.has_query4) {
		...
		if (properties_2.has_force_config) {
			retval = synaptics_rmi4_reg_read(fwu->ts_dev,
					...
					2);
			if (retval < 0) {
				TOUCH_E("%s: Failed to read tddi force block count\n",
					__func__);
				return retval;
			}
		...
		if (properties_2.has_lockdown_data) {
			retval = synaptics_rmi4_reg_read(fwu->ts_dev,
					...
					2);
			if (retval < 0) {
				TOUCH_E("%s: Failed to read tddi lockdown block count\n",
					__func__);
				return retval;
			}
		...
		if (properties_2.has_lcm_data) {
			retval = synaptics_rmi4_reg_read(fwu->ts_dev,
					...
					2);
			if (retval < 0) {
				TOUCH_E("%s: Failed to read tddi lcm block count\n",
					__func__);
				return retval;
			}
		...
		if (properties_2.has_oem_data) {
			retval = synaptics_rmi4_reg_read(fwu->ts_dev,
					...
					2);
			if (retval < 0) {
				TOUCH_E("%s: Failed to read tddi oem block count\n",
					__func__);
				return retval;
			}
		...
		unsigned short block_cnt, unsigned char command)
{
	...
	do {
		...
		retval = synaptics_rmi4_reg_write(fwu->ts_dev,
				...
				sizeof(length));
		if (retval < 0) {
			TOUCH_E("%s: Failed to write transfer length (remaining = %d)\n",
					__func__, remaining);
			return retval;
		}
		...
		if (retval < 0) {
			TOUCH_E("%s: Failed to write command (remaining = %d)\n",
					__func__, remaining);
			return retval;
		}
		...
		do {
			...
			retval = synaptics_rmi4_reg_write(fwu->ts_dev,
					...
					write_size);
			if (retval < 0) {
				TOUCH_E("%s: Failed to write block data (remaining = %d)\n",
						__func__, remaining);
				return retval;
			}
		...
		if (retval < 0) {
			TOUCH_E("%s: Failed to wait for idle status (remaining = %d)\n",
					__func__, remaining);
			return retval;
		}
		...
		unsigned char command)
{
	...
	do {
		...
		if (retval < 0) {
			TOUCH_E("%s: Failed to read block data (remaining = %d)\n",
					__func__, remaining);
			return retval;
		}
		...
		unsigned char command)
{
	...
	for (blk = 0; blk < block_cnt; blk++) {
		retval = fwu_write_f34_command(command);
		if (retval < 0) {
			TOUCH_E("%s: Failed to write read config command\n",
					__func__);
			return retval;
		}
		...
		retval = synaptics_rmi4_reg_read(fwu->ts_dev,
				...
				fwu->block_size);
		if (retval < 0) {
			TOUCH_E("%s: Failed to read block data (block %d)\n",
					__func__, blk);
			return retval;
		}
...
static int fwu_get_image_firmware_id(unsigned int *fw_id)
{
	...
	if (fwu->img.contains_firmware_id) {
		*fw_id = fwu->img.firmware_id;
		...
		if (!strptr) {
			TOUCH_E("%s: No valid PR number (PRxxxxxxx) found in image file name (%s)\n",
		...
		if (!firmware_id) {
			TOUCH_E("%s: Failed to alloc mem for firmware_id\n", __func__);
			return -ENOMEM;
		}
		...
		kfree(firmware_id);
		if (retval) {
			TOUCH_E("%s: Failed to obtain image firmware ID\n", __func__);
			return -EINVAL;
		}
	}
...
static enum flash_area fwu_go_nogo(void)
{
	...
	TOUCH_I("%s: Device firmware ID = %d\n", __func__, device_fw_id);
	...
	TOUCH_I("%s: Image firmware ID = %d\n", __func__, image_fw_id);

	if (image_fw_id > device_fw_id) {
		flash_area = UI_FIRMWARE;
		goto exit;
	} else if (image_fw_id < device_fw_id) {
		TOUCH_I("%s: Image firmware ID older than device firmware ID\n",
				__func__);
		flash_area = NONE;
		goto exit;
	}
...
exit:
	if (flash_area == NONE) {
		...
		TOUCH_I("%s: Updating %s\n", __func__, flash_area == UI_FIRMWARE ?
...
static int fwu_scan_pdt(void)
{
		...
		if (rmi_fd.fn_number) {
			...
			case SYNAPTICS_RMI4_F34:
				...
				case F34_V2:
					...
					break;
				default:
					TOUCH_E("%s: Unrecognized F34 version\n", __func__);
	...
	if (!f01found || !f34found) {
		TOUCH_E("%s: Failed to find both F01 and F34\n", __func__);
		if (!f35found) {
			TOUCH_E("%s: Failed to find F35\n", __func__);
			return -EINVAL;
		} else {
			fwu->in_ub_mode = true;
			TOUCH_I("%s: In microbootloader mode\n", __func__);
	...
	if (retval < 0) {
		TOUCH_E("%s: failed to set interrupt enable bit\n", __func__);
		return retval;
	}
...
static int synaptics_rmi4_int_enable(struct synaptics_rmi4_data *rmi4_data, bool enable)
{
	int retval = 0;
	unsigned char ii;
	...
	for (ii = 0; ii < rmi4_data->num_of_intr_regs; ii++) {
		if (intr_mask[ii] != 0x00) {
			...
			if (enable) {
				retval = synaptics_rmi4_reg_write(fwu->ts_dev, intr_addr,
						&(intr_mask[ii]), sizeof(intr_mask[ii]));
				if (retval < 0)
					return retval;
			} else {
				retval = synaptics_rmi4_reg_write(fwu->ts_dev, intr_addr,
		...
		bool enable, bool attn_only)
{
	...
	if (enable) {
		...
		if (retval < 0) {
			TOUCH_E("%s: Failed to interrupt set\n", __func__);
			goto exit;
		}
		...
		if (retval < 0) {
			TOUCH_E("%s: Failed to read interrupt status\n", __func__);
			goto exit;
		}
		...
		bool enable)
{
	...
	retval = synaptics_rmi4_irq_enable(rmi4_data, false, false);
	if (retval < 0) {
		TOUCH_E("%s: Failed to disable interrupt\n", __func__);
		goto exit;
	}

	s3706_reset_ctrl(fwu->ts_dev, SW_RESET);

	synaptics_rmi4_fwu_reset(fwu->ts_dev);
...
static int fwu_enter_flash_prog(void)
{
	...
	retval = synaptics_rmi4_irq_enable(rmi4_data, false, true);
	if (retval < 0)
		return retval;

	module_msleep(INT_DISABLE_WAIT_MS);
	...
	module_msleep(ENTER_FLASH_PROG_WAIT_MS);
...
static int fwu_check_ui_firmware_size(void)
{
	...
	if (block_count != fwu->blkcount.ui_firmware) {
		TOUCH_E("%s: UI firmware size mismatch\n", __func__);
		return -EINVAL;
	}
...
static int fwu_check_pm_configuration_size(void)
{
	...
	if (block_count != fwu->blkcount.pm_config) {
		TOUCH_E("%s: Permanent configuration size mismatch\n", __func__);
		return -EINVAL;
	}
...
static int fwu_check_bl_configuration_size(void)
{
	...
	if (block_count != fwu->blkcount.bl_config) {
		TOUCH_E("%s: Bootloader configuration size mismatch\n", __func__);
		return -EINVAL;
	}
...
static int fwu_write_utility_parameter(void)
{
	...
	for (ii = 0; ii < MAX_UTILITY_PARAMS; ii++) {
		...
		if (fwu->img.utility_param_id[ii] == FORCE_PARAMETER) {
			if (fwu->bl_mode_device) {
				TOUCH_I("%s: Device in bootloader mode, skipping calibration data restoration\n",
						__func__);
				goto image_param;
			}
			retval = secure_memcpy(&(pbuf[4]),
					...
					fwu->cal_data_size);
			if (retval < 0) {
				TOUCH_E("%s: Failed to copy force calibration data\n",
						__func__);
				return retval;
			}
...
image_param:
		...
		retval = secure_memcpy(pbuf,
				...
				fwu->img.utility_param[ii].size);
		if (retval < 0) {
			TOUCH_E("%s: Failed to copy utility parameter data\n",
					__func__);
			return retval;
		}
...
static int fwu_write_flash_configuration(void)
{
	...
	if (fwu->config_block_count != fwu->blkcount.fl_config) {
		TOUCH_E("%s: Flash configuration size mismatch\n",
				__func__);
		return -EINVAL;
	}
...
static int fwu_do_reflash(void)
{
	...
	if (!do_bl_update && fwu->incompatible_partition_tables) {
		TOUCH_E("%s: Incompatible partition tables\n", __func__);
		return -EINVAL;
...
static int fwu_do_read_config(void)
{
	...
	case DP_CONFIG_AREA:
		if (!fwu->flash_properties.has_disp_config) {
			TOUCH_E("%s: Display configuration not supported\n", __func__);
			return -EINVAL;
		}
	...
	case PM_CONFIG_AREA:
		if (!fwu->flash_properties.has_pm_config) {
			TOUCH_E("%s: Permanent configuration not supported\n", __func__);
			return -EINVAL;
		}
	...
	case BL_CONFIG_AREA:
		if (!fwu->flash_properties.has_bl_config) {
			TOUCH_E("%s: Bootloader configuration not supported\n", __func__);
			return -EINVAL;
		}
		block_count = fwu->blkcount.bl_config;
		break;
	case UPP_AREA:
		if (!fwu->has_utility_param) {
			TOUCH_E("%s: Utility parameter not supported\n", __func__);
			return -EINVAL;
		}
	...
	case TDDI_FORCE_CONFIG_AREA:
		if (!fwu->has_force_config) {
			TOUCH_E("%s: force configuration not supported\n", __func__);
			return -EINVAL;
		}
	...
	case TDDI_OEM_DATA_AREA:
		if (!fwu->has_oem_data) {
			TOUCH_E("%s: oem data not supported\n", __func__);
			return -EINVAL;
		}
	...
	case TDDI_LCM_DATA_AREA:
		if (!fwu->has_lcm_data) {
			TOUCH_E("%s: lcm data not supported\n", __func__);
			return -EINVAL;
		}
...
static int fwu_do_read_tddi_lockdown_data(void)
{
	...
	if (fwu->bl_version != BL_V6) {
		TOUCH_E("%s: Not support lockdown data in bl v.%d\n",
				__func__, fwu->bl_version);
		goto exit;
	} else if (!fwu->has_lockdown_data) {
		TOUCH_E("%s: Not support lockdown data\n", __func__);
		goto exit;
	}
...
int set_tddi_lockdown_data(unsigned char *lockdown_data, unsigned short leng)
{
	...
	if (retval < 0) {
		TOUCH_E("%s: Failed to copy tddi lockdwon data\n", __func__);
		goto exit;
	}
...
static int fwu_do_lockdown_v7(void)
{
	...
	if (status.device_cfg_status == 2) {
		TOUCH_I("%s: Device already locked down\n", __func__);
		return 0;
	}
...
static int fwu_do_lockdown_v5v6(void)
{
	...
	if (fwu->has_lockdown_data) {
		retval = set_tddi_lockdown_data(img_ld,
				LOCKDOWN_SIZE);
		if (retval < 0)
			TOUCH_E("%s: Failed to write lockdown data\n", __func__);
...
static int fwu_do_restore_f51_cal_data(void)
{
	...
	retval = secure_memcpy(&fwu->read_config_buf[fwu->cal_data_off],
			...
			fwu->cal_data_buf_size, fwu->cal_data_size);
	if (retval < 0) {
		TOUCH_E("%s: Failed to restore calibration data\n", __func__);
		return retval;
	}
...
static int fwu_start_write_guest_code(void)
{
	...
	if (!fwu->has_guest_code) {
		TOUCH_E("%s: Guest code not supported\n", __func__);
		return -EINVAL;
	}

	if (!fwu->img.contains_guest_code) {
		TOUCH_E("%s: No guest code in firmware image\n", __func__);
		return -EINVAL;
	}

	if (rmi4_data->sensor_sleep) {
		TOUCH_E("%s: Sensor sleeping\n", __func__);
		return -ENODEV;
	}
	...
	TOUCH_I("%s: Start of write guest code process\n", __func__);
...
exit:
	...
	TOUCH_I("%s: End of write guest code process\n", __func__);
...
static int fwu_start_write_config(void)
{
	...
	case UI_CONFIG_AREA:
		...
		if (device_fw_id != image_fw_id) {
			TOUCH_E("%s: Device and image firmware IDs don't match\n", __func__);
			return -EINVAL;
		}
		retval = fwu_check_ui_configuration_size();
		if (retval < 0)
	...
	case DP_CONFIG_AREA:
		...
		if (!fwu->img.contains_disp_config) {
			TOUCH_E("%s: No display configuration in firmware image\n", __func__);
			return -EINVAL;
		}
		retval = fwu_check_dp_configuration_size();
		if (retval < 0)
	...
	case PM_CONFIG_AREA:
		...
		if (!fwu->img.contains_perm_config) {
			TOUCH_E("%s: No permanent configuration in firmware image\n", __func__);
			return -EINVAL;
		}
		...
		break;
	default:
		TOUCH_E("%s: Configuration not supported\n", __func__);
		return -EINVAL;
	}
	...
	TOUCH_I("%s: Start of write config process\n", __func__);
	...
	if (fwu->config_area != PM_CONFIG_AREA) {
		retval = fwu_erase_configuration();
		if (retval < 0) {
			TOUCH_E("%s: Failed to erase config\n", __func__);
			goto exit;
		}
	}
	...
	TOUCH_I("%s: Config written\n", __func__);

exit:
	switch (fwu->config_area) {
	case UI_CONFIG_AREA:
		synaptics_rmi4_reset_device(rmi4_data, true);
		break;
	case DP_CONFIG_AREA:
	case PM_CONFIG_AREA:
		synaptics_rmi4_reset_device(rmi4_data, false);
	...
	TOUCH_I("%s: End of write config process\n", __func__);
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		...
		if (retval != 0) {
			TOUCH_E("%s: Firmware image %s not available\n",
					__func__, fwu->image_name);
			retval = -EINVAL;
			goto exit;
		}

		TOUCH_I("%s: Firmware image size = %d\n", __func__,
	...
	if (fwu->in_bl_mode) {
		...
		TOUCH_I("%s: Device in bootloader mode\n", __func__);
	} else {
	...
	if (flash_area != NONE && !fwu->bl_mode_device) {
		...
		if (retval < 0) {
			TOUCH_E("%s: Failed to save calibration data\n",
					__func__);
			synaptics_rmi4_reset_device(rmi4_data, false);
			goto exit;
		}
...
static int fwu_recovery_check_status(void)
{
	...
	retval = synaptics_rmi4_reg_read(fwu->ts_dev,
			...
			1);
	if (retval < 0) {
		TOUCH_E("%s: Failed to read status\n", __func__);
		return retval;
	}
	...
	if (status != 0x00) {
		TOUCH_E("%s: Recovery mode status = %d\n", __func__, status);
		return -EINVAL;
	}
...
static int fwu_recovery_erase_completion(void)
{
	...
	do {
		...
		retval = synaptics_rmi4_reg_write(fwu->ts_dev,
				...
				sizeof(command));
		if (retval < 0) {
			TOUCH_E("%s: Failed to issue command\n", __func__);
			return retval;
		}
		...
		do {
			retval = synaptics_rmi4_reg_read(fwu->ts_dev,
					...
					sizeof(command));
			if (retval < 0) {
				TOUCH_E("%s: Failed to read command status\n", __func__);
				return retval;
			}
...
exit:
	if (timeout == 0) {
		TOUCH_E("%s: Timed out waiting for flash erase completion\n",
				__func__);
		return -ETIMEDOUT;
	}
...
static int fwu_recovery_erase_all(void)
{
	...
	retval = synaptics_rmi4_reg_write(fwu->ts_dev,
			...
			sizeof(command));
	if (retval < 0) {
		TOUCH_E("%s: Failed to issue erase all command\n", __func__);
		return retval;
	}
	...
	if (fwu->f35_fd.cmd_base_addr) {
		...
		module_msleep(F35_ERASE_ALL_WAIT_MS);
	}
...
static int fwu_recovery_write_chunk(void)
{
	...
	retval = synaptics_rmi4_reg_write(fwu->ts_dev,
			...
			sizeof(chunk_number));
	if (retval < 0) {
		TOUCH_E("%s: Failed to write chunk number\n", __func__);
		return retval;
	}
	...
	for (chunk = 0; chunk < chunk_total; chunk++) {
		...
		retval = synaptics_rmi4_reg_write(fwu->ts_dev,
				...
				sizeof(buf));
		if (retval < 0) {
			TOUCH_E("%s: Failed to write chunk data (chunk %d)\n",
					__func__, chunk);
			return retval;
		}
	...
	retval = fwu_recovery_check_status();
	if (retval < 0) {
		TOUCH_E("%s: Failed to write chunk data\n", __func__);
		return retval;
	}
...
static int fwu_recovery_reset(void)
{
	...
	retval = synaptics_rmi4_reg_write(fwu->ts_dev,
			...
			sizeof(command));
	if (retval < 0) {
		TOUCH_E("%s: Failed to issue reset command\n", __func__);
		return retval;
	}

	module_msleep(F35_RESET_WAIT_MS);
...
static int fwu_start_recovery(void)
{
	...
	if (fwu->image == NULL) {
		...
		retval = request_firmware(&fw_entry, fwu->image_name,
				rmi4_data->pdev->dev.parent);
		if (retval != 0) {
			TOUCH_E("%s: Firmware ihex %s not available\n",
					__func__, fwu->image_name);
			retval = -EINVAL;
			goto exit;
		}
	...
	if (retval < 0) {
		TOUCH_E("%s: Failed to do erase all in recovery mode\n",
				__func__);
		goto exit;
	}

	TOUCH_I("%s: External flash erased\n", __func__);

	retval = fwu_recovery_write_chunk();
	if (retval < 0) {
		TOUCH_E("%s: Failed to write chunk data in recovery mode\n",
				__func__);
		goto exit;
	}

	TOUCH_I("%s: Chunk data programmed\n", __func__);

	retval = fwu_recovery_reset();
	if (retval < 0) {
		TOUCH_E("%s: Failed to reset device in recovery mode\n",
				__func__);
		goto exit;
	}

	TOUCH_I("%s: Recovery mode reset issued\n", __func__);
...
exit:
	...
	TOUCH_I("%s: End of recovery process\n", __func__);
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	while (!rmi4_data->fb_ready) {
		module_msleep(FB_READY_WAIT_MS);
		timeout--;
		if (timeout == 0) {
			TOUCH_E("%s: Timed out waiting for FB ready\n", __func__);
			return;
		}
	}
#endif
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (count < fwu->config_size) {
		TOUCH_E("%s: Not enough space (%d bytes) in buffer\n",
	...
	retval = secure_memcpy(buf, count, fwu->read_config_buf,
			fwu->read_config_buf_size, fwu->config_size);
	if (retval < 0) {
		TOUCH_E("%s: Failed to copy config data\n", __func__);
		goto exit;
	} else {
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (retval < 0) {
		TOUCH_E("%s: Failed to copy image data\n", __func__);
		goto exit;
	} else {
		retval = count;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!fwu->in_ub_mode) {
		TOUCH_E("%s: Not in microbootloader mode\n", __func__);
		retval = -EINVAL;
		goto exit;
	}
	...
	retval = fwu_start_recovery();
	if (retval < 0) {
		TOUCH_E("%s: Failed to do recovery\n", __func__);
		goto exit;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (fwu->in_ub_mode) {
		TOUCH_E("%s: In microbootloader mode\n", __func__);
		retval = -EINVAL;
		goto exit;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval < 0) {
		TOUCH_E("%s: Failed to write config\n", __func__);
		goto exit;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval < 0) {
		TOUCH_E("%s: Failed to read config\n", __func__);
		goto exit;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!fwu->ext_data_source) {
		TOUCH_E("%s: Failed to alloc mem for image data\n", __func__);
		retval = -ENOMEM;
	} else {
		retval = count;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval < 0) {
		TOUCH_E("%s: Failed to write guest code\n", __func__);
		goto exit;
	}
...
static int synaptics_rmi4_reg_read(struct device *dev,
		unsigned short addr, unsigned char *data, unsigned int len)
{
	return s3706_read(dev, addr, data, len);
}

static int synaptics_rmi4_reg_write(struct device *dev,
		unsigned short addr, unsigned char *data, unsigned int len)
{
	return s3706_write(dev, addr, data, len);
}
...
void synaptics_rmi4_fwu_attn(struct device *ts_dev,
		unsigned char intr_mask)
{
	if (!fwu)
		return;
...
int synaptics_rmi4_fwu_init(struct device *ts_dev)
{
	struct module_data *md = to_module(ts_dev);
	struct s3706_data *d = to_s3706_data(ts_dev);
	...
	if (!fwu->image_name) {
		TOUCH_E("%s: Failed to alloc mem for image name\n", __func__);
		retval = -ENOMEM;
		goto exit_free_fwu;
	}
	...
	if (retval < 0) {
		TOUCH_E("%s: Failed to read PDT properties, assuming 0x00\n", __func__);
	} else if (pdt_props.has_bsr) {
		TOUCH_E("%s: Reflash for LTS not currently supported\n", __func__);
		retval = -ENODEV;
		goto exit_free_mem;
	}
	...
	if (!rmi4_data) {
		TOUCH_E("%s: Failed to alloc mem for rmi4_data\n", __func__);
		retval = -ENOMEM;
		goto exit_free_mem;
	}
	...
	retval = sysfs_create_bin_file(&md->input->dev.kobj,
			&dev_attr_data);
	if (retval < 0) {
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		retval = sysfs_create_file(&md->input->dev.kobj,
				&attrs[attr_count].attr);
		if (retval < 0) {
			TOUCH_I("%s: Failed to create sysfs attributes\n", __func__);
			retval = -ENODEV;
...
exit_remove_attrs:
	for (attr_count--; attr_count >= 0; attr_count--) {
		sysfs_remove_file(&md->input->dev.kobj,
				&attrs[attr_count].attr);
	}
...
void synaptics_rmi4_fwu_remove(struct device *ts_dev)
{
	...
	sysfs_remove_bin_file(&md->input->dev.kobj, &dev_attr_data);
...
static void synaptics_rmi4_fwu_reset(struct device *ts_dev)
{
	...
	if (!fwu) {
		synaptics_rmi4_fwu_init(ts_dev);
		return;
	}
...
struct s3706_fwu_exp_fn fwu_module = {
...
{
	s3706_fwu_function(&fwu_module, true);
...
{
	s3706_fwu_function(&fwu_module, false);
