...
static int spec_file_read(struct device *dev)
{
	...
	if (is_ddic_name("rm69299a")) {
		...
		path[0] = ts->dual_panel_spec[DS_TIANMA_PATH];
		path[1] = ts->dual_panel_spec_mfts[DS_TIANMA_PATH];
	} else if (is_ddic_name("rm69299b")) {
		TOUCH_I("set dual display Tovis panel spec path\n");
		path[0] = ts->dual_panel_spec[DS_TOVIS_PATH];
		path[1] = ts->dual_panel_spec_mfts[DS_TOVIS_PATH];
	} else {
		TOUCH_I("set Flash spec path\n");
		path[0] = ts->dual_panel_spec[MAIN_PATH];
		path[1] = ts->dual_panel_spec_mfts[MAIN_PATH];
	}
			...
			unsigned char tx, unsigned char rx, int *buf)
{
	...
	//add for only dual display test
...
static int production_info_check(struct device *dev)
{
	...
	//check for dual display test
	if (s3707_is_product(d, "PLG670", 6) ||
			s3707_is_product(d, "PLG676", 6) ||
			s3707_is_product(d, "S3706B", 6)) {
		TOUCH_I("%s: Dual Display don't use this func\n", __func__);
		return ret;
	}
...
static int s3707_sd(struct device *dev, char *buf)
{
	...
	//add temporary code due to unspecified spec of Tovis
	if (is_ddic_name("rm69299b")) {
		rawdata_ret = 1;
		jitter_ret = 1;
		hybrid_abs_ret = 1;
		trx_short_ret = 1;
		high_resistance_ret = 1;
	}
	//
...
static ssize_t show_lpwg_sd(struct device *dev, char *buf)
{
	...
	if (s3707_is_product(d, "PLG670", 6) ||
			s3707_is_product(d, "PLG676", 6) ||
			s3707_is_product(d, "S3706B", 6)) {
		TOUCH_I("Dual Display don't use lpwg sd test\n");
		return ret;
	}
