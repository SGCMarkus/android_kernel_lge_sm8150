...
int trans_rx_short_limit[TRX_MAX];
...
static int spec_file_read(struct device *dev)
{
	...
	if (ts->dual_panel_spec == NULL || ts->dual_panel_spec_mfts == NULL) {
...
static int clear_buf(void)
{
	...
	for (i = 0; i < TRX_MAX; i++) {
		memset(rawcap_upper[i], 0, sizeof(int) * TRX_MAX);
		memset(rawcap_lower[i], 0, sizeof(int) * TRX_MAX);
	}

	memset(hybrid_abs_rx_upper, 0, sizeof(int) * TRX_MAX);
	memset(hybrid_abs_rx_lower, 0, sizeof(int) * TRX_MAX);
	memset(hybrid_abs_tx_upper, 0, sizeof(int) * TRX_MAX);
	memset(hybrid_abs_tx_lower, 0, sizeof(int) * TRX_MAX);
	memset(trx_short_limit, 0, sizeof(int) * TRX_BITMAP_LENGTH);
	memset(ext_trx_short_limit, 0, sizeof(int) * 2);
	jitter_upper = 0;
	jitter_lower = 0;
	high_resistance_upper = 0;
	high_resistance_lower = 0;

	TOUCH_I("buffer clear done.\n");
	return 0;
}
...
static int s3706_sd(struct device *dev, char *buf)
{
	...
	lower_limit += s3706_get_limit(dev, "TRANS_RX_SHORT_LIMIT",
				1, RxChannelCount, (int *)trans_rx_short_limit);
	...
	if (lower_limit < 0 || upper_limit < 0) {
		...
		touch_msleep(50);
		trans_rx_short_ret = F54Test(dev, eRT_TransRxShortUsingCBCVariation , 0, NULL);
		touch_msleep(50);
	}
...
static ssize_t show_trans_rx_short(struct device *dev, char *buf)
{
	struct touch_core_data *ts  = to_touch_core(dev);
	struct s3706_data *d = to_s3706_data(dev);
	...
	if (s3706_is_product(d, "PLG661", 6) ||
			s3706_is_product(d, "PLG674", 6) ||
			s3706_is_product(d, "PLG680", 6) ||
		...
		ret += touch_snprintf(buf + ret, PAGE_SIZE - ret, "Trans Rx Short Test result : %s",
					(trans_rx_short_ret == 1) ? "Pass\n" : "Fail\n");
	} else {
		TOUCH_E("Trans Rx Short Test Error!!!(product_id error)\n");
		ret += touch_snprintf(buf + ret, PAGE_SIZE - ret, "-1\n");
		ret += touch_snprintf(buf + ret, PAGE_SIZE - ret, "Read Fail Touch IC Info\n");
	}
...
static TOUCH_ATTR(trans_rx_short, show_trans_rx_short, NULL);
static struct attribute *prd_attribute_list[] = {
	...
	&touch_attr_trans_rx_short.attr,
	NULL,
};
