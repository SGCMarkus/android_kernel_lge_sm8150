...
int s3706_read(struct device *dev, u8 addr, void *data, int size)
{
	...
	// This function returns one or more if successful.
	if (ret < 0) {
...
int s3706_set_page(struct device *dev, u8 page)
{
	...
	ret = s3706_write(dev, PAGE_SELECT_REG, &page, 1);
	if (ret < 0) {
		TOUCH_E("failed to write page (ret: %d)\n", ret);
		return -EINVAL;
	}
...
static int s3706_get_production_info(struct device *dev)
{
	...
	for (i = 0; i <= PRD_BLOCK_MAX; i++) {
		...
		if (curr_data[0] != 0) {
			...
			if (!strncmp(d->prd_info.product_id, "PLG674", 6) ||
					!strncmp(d->prd_info.product_id, "PLG680", 6))
				write_count++;
		} else {
			if (i != 0)
				break;
...
int s3706_ic_info(struct device *dev)
{
	...
	snprintf(str, sizeof(str), "%s", d->ic_info.product_id);
...
error:
	...
	return ret;
	*/
	TOUCH_I("Do nothing due to fail reason register deleted\n");
	return 0;
}
...
static int s3706_tci_knock(struct device *dev)
{
	...
	if ((ts->tci.mode & (1 << i)) == 0x0) {
		lpwg_data[0] = 0;
		s3706_write(dev, LPWG_TAPCOUNT_REG, lpwg_data, sizeof(u8));
	} else {
		info = &ts->tci.info[i];
		...
		lpwg_data[4] = info->tap_distance;
		lpwg_data[6] = (info->intr_delay << 1 | 1);
		s3706_write(dev, LPWG_TAPCOUNT_REG, lpwg_data, sizeof(lpwg_data));
	}
#endif
...
static int s3706_lpwg_longpress_enable(struct device *dev, bool enable)
{
	struct s3706_data *d = to_s3706_data(dev);
	int ret = 0;
	int ret_page = 0;
	...
	if (enable) {
		buf[0] = d->lpwg_longpress.enable;
		buf[1] = (d->lpwg_longpress.area.x1) & 0xFF;
		buf[2] = (d->lpwg_longpress.area.x1) >> 8;
		buf[3] = (d->lpwg_longpress.area.y1) & 0xFF;
		buf[4] = (d->lpwg_longpress.area.y1) >> 8;
		buf[5] = (d->lpwg_longpress.area.x2) & 0xFF;
		buf[6] = (d->lpwg_longpress.area.x2) >> 8;
		buf[7] = (d->lpwg_longpress.area.y2) & 0xFF;
		buf[8] = (d->lpwg_longpress.area.y2) >> 8;
		buf[9] = d->lpwg_longpress.min_press_time;
		buf[10] = d->lpwg_longpress.min_contact_size;
		buf[11] = d->lpwg_longpress.touch_slop;

		ret = s3706_write(dev, LPWG_LONGPRESS_ENABLE_REG, (u8 *)buf, sizeof(buf));
		if (ret < 0) {
			TOUCH_E("failed to set LPWG_LONGPRESS_ENABLE_REG(ret = %d)\n", ret);
			goto error;
		}
	} else {
		TOUCH_I("Do nothing for longpress enable!\n");
...
static int s3706_lpwg_onetap_enable(struct device *dev, bool enable)
{
	struct s3706_data *d = to_s3706_data(dev);
	int ret = 0;
	int ret_page = 0;
	...
	if (enable) {
		buf[0] = d->lpwg_onetap.enable;
		buf[1] = (d->lpwg_onetap.area.x1) & 0xFF;
		buf[2] = (d->lpwg_onetap.area.x1) >> 8;
		buf[3] = (d->lpwg_onetap.area.y1) & 0xFF;
		buf[4] = (d->lpwg_onetap.area.y1) >> 8;
		buf[5] = (d->lpwg_onetap.area.x2) & 0xFF;
		buf[6] = (d->lpwg_onetap.area.x2) >> 8;
		buf[7] = (d->lpwg_onetap.area.y2) & 0xFF;
		buf[8] = (d->lpwg_onetap.area.y2) >> 8;
		buf[9] = d->lpwg_onetap.touch_slop;
		buf[10] = d->lpwg_onetap.max_press_time;
		buf[11] = d->lpwg_onetap.intr_delay;

		ret = s3706_write(dev, LPWG_ONETAP_ENABLE_REG, (u8 *)buf, sizeof(buf));
		if (ret < 0) {
			TOUCH_E("failed to set LPWG_ONETAP_ENABLE_REG(ret = %d)\n", ret);
			goto error;
		}
	} else {
		TOUCH_I("Do nothing for onetap enable!\n");
...
static void s3706_longpress_release(struct device *dev) {
	struct touch_core_data *ts = to_touch_core(dev);
	struct s3706_data *d = to_s3706_data(dev);
...
static int s3706_lpwg_mode(struct device *dev)
{
		...
		if (ts->lpwg.screen) {
			...
			if (ts->lpwg.mode == LPWG_NONE
				...
				if (d->lpwg_longpress.enable) {
					ret = s3706_lpwg_longpress_enable(dev, d->lpwg_longpress.enable);
					if (ret < 0) {
						TOUCH_E("failed to set d->lpwg_longpress enable (ret: %d)\n", ret);
						goto error;
					}
				}
				if (d->lpwg_onetap.enable) {
					ret = s3706_lpwg_onetap_enable(dev, d->lpwg_onetap.enable);
					if (ret < 0) {
						TOUCH_E("failed to set d->lpwg_onetap enable (ret: %d)\n", ret);
						goto error;
					}
				}
			}
	...
	if (ts->lpwg.screen) {
		...
		// Do not set longpress / onetap in partial timing
	}
...
static void s3706_init_swipe_info(struct device *dev)
{
	...
	ts->swipe[SWIPE_U].area.x1 = 0 + ACT_SENSELESS_AREA_W;
	ts->swipe[SWIPE_U].area.y1 = 0;
	ts->swipe[SWIPE_U].area.x2 = ts->caps.max_x - ACT_SENSELESS_AREA_W;
...
static void s3706_init_longpress_info(struct device *dev)
{
	struct s3706_data *d = to_s3706_data(dev);

	TOUCH_TRACE();

	d->lpwg_longpress.enable = 0;
...
static void s3706_init_onetap_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct s3706_data *d = to_s3706_data(dev);
...
static int s3706_lpwg_longpress_getdata(struct device *dev, u8 gesture_type)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct s3706_data *d = to_s3706_data(dev);
	u8 buffer[10] = {0,};
	...
	ret = s3706_read(dev, LONGPRESS_X_REG, buffer, sizeof(buffer));
	if (ret < 0) {
		TOUCH_E("failed to read longpress registers (ret: %d)\n", ret);
		ts->intr_status = TOUCH_IRQ_ERROR;
		return ret;
	}

	switch (gesture_type) {
	case LPWG_STATUS_LONGP_DOWN:
	case LPWG_STATUS_LONGP_UP:
		coord_x = buffer[0] + ((buffer[1] & 0xFF) << 8);
		coord_y = buffer[2] + ((buffer[3] & 0xFF) << 8);
		minor_width = buffer[5] + ((buffer[4] & 0xFF) << 8);
		TOUCH_I("x: %d, y: %d, minor_width: %d\n", coord_x, coord_y, minor_width);
		break;
	case LPWG_STATUS_ONETAP:
		coord_x = buffer[6] + ((buffer[7] & 0xFF) << 8);
		coord_y = buffer[8] + ((buffer[9] & 0xFF) << 8);
		TOUCH_I("x: %d, y: %d\n", coord_x, coord_y);
		break;
	default :
		TOUCH_I("unsupport type : %X", gesture_type);
		break;
	}
...
static int s3706_irq_lpwg(struct device *dev)
{
	...
	} else if (status & LPWG_STATUS_LONGP_DOWN) {
		TOUCH_I("LONGP_DOWN\n");
		ret = s3706_lpwg_longpress_getdata(dev, LPWG_STATUS_LONGP_DOWN);
		if (ret < 0) {
			TOUCH_E("failed to LONGP_D getdata (reg: %d)\n", ret);
			goto error;
		}
		ts->intr_status = TOUCH_IRQ_LPWG_LONGPRESS_DOWN;
		d->longpress_uevent_status = TOUCH_IRQ_LPWG_LONGPRESS_DOWN;
	} else if (status & LPWG_STATUS_LONGP_UP) {
		TOUCH_I("LONGP_UP\n");
		ret = s3706_lpwg_longpress_getdata(dev, LPWG_STATUS_LONGP_UP);
		if (ret < 0) {
			TOUCH_E("failed to LONGP_U getdata (reg: %d)\n", ret);
			goto error;
		}
		ts->intr_status = TOUCH_IRQ_LPWG_LONGPRESS_UP;
		d->longpress_uevent_status = TOUCH_IRQ_LPWG_LONGPRESS_UP;
	} else if (status & LPWG_STATUS_ONETAP) {
		TOUCH_I("ONETAP\n");
		ret = s3706_lpwg_longpress_getdata(dev, LPWG_STATUS_ONETAP);
		if (ret < 0) {
			TOUCH_E("failed to ONETAP getdata (reg: %d)\n", ret);
			goto error;
		}
...
static int s3706_get_cmd_version(struct device *dev, char *buf)
{
	...
	if (s3706_is_product(d, "PLG674", 6) || s3706_is_product(d, "PLG680", 6))
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
			"Touch IC : S3706\n\n");
	else
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
	...
	if (s3706_is_img_product(d, "PLG674", 6) || s3706_is_img_product(d, "PLG680", 6))
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
			"Touch IC : S3706\n\n");
	else
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
...
static ssize_t show_lpwg_longpress(struct device *dev, char *buf)
{
	struct s3706_data *d = to_s3706_data(dev);
	int ret = 0;
	...
	TOUCH_I("%s: d->lpwg_longpress = %d\n", __func__, d->lpwg_longpress.enable);

	return ret;
}

static ssize_t store_lpwg_longpress(struct device *dev,
		const char *buf, size_t count)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct s3706_data *d = to_s3706_data(dev);
...
static ssize_t show_lpwg_onetap(struct device *dev, char *buf)
{
	struct s3706_data *d = to_s3706_data(dev);
	int ret = 0;
	...
	TOUCH_I("%s: d->lpwg_onetap = %d\n", __func__, d->lpwg_onetap.enable);

	return ret;
}

static ssize_t store_lpwg_onetap(struct device *dev,
		const char *buf, size_t count)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct s3706_data *d = to_s3706_data(dev);
...
static TOUCH_ATTR(longpress, show_lpwg_longpress, store_lpwg_longpress);
static TOUCH_ATTR(onetap, show_lpwg_onetap, store_lpwg_onetap);
...
static int s3706_fw_compare(struct device *dev, const struct firmware *fw)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct s3706_data *d = to_s3706_data(dev);
	struct s3706_version *ic_version = &d->ic_info.version;
	struct s3706_version *img_version = &d->ic_info.img_version;

	int update = UPGRADE_NO_NEED;

	TOUCH_TRACE();

	memset(img_version, 0, sizeof(struct s3706_version));

	memcpy(d->ic_info.img_product_id, &fw->data[d->ic_info.fw_pid_addr], 6);
	memcpy(d->ic_info.img_raws, &fw->data[d->ic_info.fw_ver_addr], 4);
	img_version->major = (d->ic_info.img_raws[3] & 0x80 ? 1 : 0);
	img_version->minor = (d->ic_info.img_raws[3] & 0x7F);

	if (ts->force_fwup) {
		update = UPGRADE_FORCE;
	} else if (img_version->major != ic_version->major) {
		update = UPGRADE_NEED;
	} else if (img_version->minor != ic_version->minor) {
		update = UPGRADE_NEED;
	}

	TOUCH_I("%s: img_version[%d.%02d] ic_version[%d.%02d] -> update: %d, force: %d\n",
			__func__, img_version->major, img_version->minor,
...
static int s3706_upgrade(struct device *dev)
{
	...
	if ((ts->force_fwup == UPGRADE_NO_NEED) && (atomic_read(&d->state.init) == IC_INIT_NEED)) {
	...
	d->check_fwup = s3706_fw_compare(dev, fw);
	...
	switch (boot_mode) {
		...
		case TOUCH_MINIOS_MFTS_CURVED:
			// DS2 Firmware for MFTS
			if (d->check_fwup == UPGRADE_NEED) {
				if (is_ddic_name("rm692A9")) {
					TOUCH_I("Need touch firmware force upgrade of dual display panel.\n");
					goto error;
				} else {
					atomic_set(&d->state.scan_pdt, true);
			...
			} else if (d->check_fwup == UPGRADE_FORCE) {
...
static void s3706_init_locks(struct device *dev)
{
	struct s3706_data *d = to_s3706_data(dev);
...
static int s3706_probe(struct device *dev)
{
	...
	d->noise.noise_log = NOISE_ENABLE;
	d->check_fwup = UPGRADE_NOT_INITIALIZED;

	d->lcd_mode = LCD_MODE_U3;

	// Dts specific settings
	ts->caps.hw_reset_delay = 100;
	ts->caps.sw_reset_delay = 100;
#if defined(CONFIG_SECURE_TOUCH)
	ts->touch_ic_name = "synaptics,s3706";
#endif
...
static int __init touch_device_init(void)
{
	...
	if (is_ddic_name("rm692A9")) {
		TOUCH_I("%s, rm692A9 Tovis s3706 detected\n", __func__);
	} else {
