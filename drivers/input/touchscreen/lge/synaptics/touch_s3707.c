	...
	for (i = 0; i <= PRD_BLOCK_MAX; i++) {
		...
		if (curr_data[0] != 0) {
			...
			if (!strncmp(d->prd_info.product_id, "PLG661", 6) ||
					!strncmp(d->prd_info.product_id, "PLG670", 6) ||
					!strncmp(d->prd_info.product_id, "PLG676", 6) ||
					!strncmp(d->prd_info.product_id, "S3706B", 6)) {
				write_count++;
			}
		} else {
			if (i != 0)
...
static int s3707_swipe_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct s3707_data *d = to_s3707_data(dev);
	struct swipe_ctrl *ctrl[S3707_SWIPE_NUM] = {	/* U, L, R, L2, R2 */
		&ts->swipe[SWIPE_U],
		&ts->swipe[SWIPE_L],
		&ts->swipe[SWIPE_R],
		&ts->swipe[SWIPE_L2],
	...
	TOUCH_I("%s: SWIPE U(%d),L(%d),R(%d),L2(%d),R2(%d)\n", __func__,
			ctrl[S3707_SWIPE_U]->enable,
			ctrl[S3707_SWIPE_L]->enable,
			ctrl[S3707_SWIPE_R]->enable,
			ctrl[S3707_SWIPE_L2]->enable,
			ctrl[S3707_SWIPE_R2]->enable);

	if (enable) {
		memset(&buf, 0, sizeof(buf));

		for (i = 0; i < S3707_SWIPE_NUM; i++) {	/* U, L, R, L2, R2 */
		...
		ret = s3707_write(dev, SWIPE_ENABLE_REG, (u8 *)&buf, sizeof(buf));
		if (ret < 0)
			TOUCH_E("failed to set SWIPE_ENABLE_REG (ret: %d)\n", ret);
	} else {
		for (i = 0; i < S3707_SWIPE_NUM; i++)
			ctrl[i]->debug_enable = false;

		memset(&(buf.enable), 0, sizeof(buf.enable));

		ret = s3707_write(dev, SWIPE_ENABLE_REG, (u8 *)&(buf.enable),
				sizeof(buf.enable));
		if (ret < 0)
...
static int s3707_lpwg_mode(struct device *dev)
{
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		if (ts->mfts_lpwg) {
			...
			ret = s3707_swipe_enable(dev, true);
			if (ret < 0) {
...
static int s3707_irq_abs(struct device *dev)
{
	...
	ret = s3707_get_finger_count(dev);
	if (ret < 0) {
...
static int s3707_esd_recovery(struct device *dev)
{
	TOUCH_TRACE();
...
static int s3707_bin_fw_version(struct device *dev)
{
	...
	ret = request_firmware(&fw, ts->def_fwpath[fw_path], dev);
	if (ret) {
...
static int s3707_get_cmd_version(struct device *dev, char *buf)
{
	...
	if (s3707_is_product(d, "PLG661", 6))
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
			"Touch IC : S3707\n\n");
	else if (s3707_is_product(d, "PLG670", 6) || s3707_is_product(d, "S3706B", 6))
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
			"DD Touch IC : S3706(Tianma)\n\n");
	else if (s3707_is_product(d, "PLG676", 6))
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
			"DD Touch IC : S3706(Tovis)\n\n");
	else
		offset += snprintf(buf + offset, PAGE_SIZE - offset,
...
static int s3707_wireless_status(struct device *dev, u32 mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int s3707_film_status(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct s3707_data *d = to_s3707_data(dev);
	...
	ret = s3707_read(dev, SENSITIVE_MODE_ENABLE_REG, &buffer, sizeof(buffer));
	if (ret < 0) {
	...
	ret = s3707_write(dev, SENSITIVE_MODE_ENABLE_REG, &buffer, sizeof(buffer));
	if (ret < 0)
...
static void s3707_grip_status(struct device *dev, u32 *data)
{
	struct s3707_data *d = to_s3707_data(dev);
	int grip_status = *data;

	if (grip_status)
		atomic_set(&d->state.grip_status, GRIP_SUPPRESSION_ENABLE);
	else
		atomic_set(&d->state.grip_status, GRIP_SUPPRESSION_DISABLE);
}

static int s3707_grip_set(struct device *dev)
{
	struct s3707_data *d = to_s3707_data(dev);
	int ret = 0;
	int data = atomic_read(&d->state.grip_status);
	int dd_state = atomic_read(&d->state.dd_status);
	u8 buffer = 0;
...
static int s3707_notify(struct device *dev, ulong event, void *data)
{
	...
	case NOTIFY_WIRELESS:
		...
		ret = s3707_wireless_status(dev, *(u32 *)data);
		if (ret < 0)
			TOUCH_E("failed to set wireless status (ret: %d)\n", ret);
		break;
	...
	case NOTIFY_FILM_STATE:
		...
		ret = s3707_film_status(dev);
		if (ret < 0)
			TOUCH_E("failed to set film status (ret: %d)\n", ret);
		break;
	case NOTIFY_DD_HALLIC_STATE:
		TOUCH_I("%s: NOTIFY_DD_HALLIC_STATE!\n", __func__);
		s3707_grip_status(dev, &grip_data);
		if (!grip_data)
			atomic_set(&d->state.dd_status, DD_CONNECTED);
		else
			atomic_set(&d->state.dd_status, DD_DISCONNECTED);
		ret = s3707_grip_set(dev);
		if (ret < 0)
			TOUCH_E("failed to set grip set (ret: %d)\n", ret);
		break;
	default:
...
static int s3707_init_pm(struct device *dev)
{
#if defined(CONFIG_DRM) && defined(CONFIG_FB)
#if defined(CONFIG_LGE_TOUCH_USE_PANEL_NOTIFY)
	struct touch_core_data *ts = to_touch_core(dev);
...
static int s3707_upgrade(struct device *dev)
{
	...
	if (ts->test_fwpath[0]) {
		...
		if (is_ddic_name("rm69299a"))
			memcpy(fwpath, ts->def_fwpath[DS_TIANMA_PATH], sizeof(fwpath));
		else if (is_ddic_name("rm69299b"))
			memcpy(fwpath, ts->def_fwpath[DS_TOVIS_PATH], sizeof(fwpath));
		else
			memcpy(fwpath, ts->def_fwpath[MAIN_PATH], sizeof(fwpath));
	...
	d->check_fwup = s3707_fw_compare(dev, fw);
...
static int s3707_resume(struct device *dev)
{
	...
	touch_interrupt_control(dev, INTERRUPT_DISABLE);
	s3707_reset_ctrl(dev, SW_RESET_NO_INIT);
...
int s3707_init(struct device *dev)
{
	...
	ret = s3707_grip_set(dev);
	if (ret < 0) {
		TOUCH_E("failed to set grip status (ret: %d)\n", ret);
		goto error;
	}
...
static int s3707_probe(struct device *dev)
{
	...
	d->noise.noise_log = NOISE_ENABLE;
	d->check_fwup = UPGRADE_NOT_INITAILIZED;

	d->lcd_mode = LCD_MODE_U3;
	atomic_set(&d->state.grip_status, GRIP_SUPPRESSION_ENABLE);

	if (is_ddic_name("rm69299a") || is_ddic_name("rm69299b")) {
		TOUCH_I("%s: change max_x, max_y for DD panel.\n", __func__);
		ts->caps.max_x = 1079;
		ts->caps.max_y = 2159;
	}
...
static struct touch_driver touch_driver = {
	...
	.lpwg = s3707_lpwg,
	.notify = s3707_notify,
	.init_pm = s3707_init_pm,
	.esd_recovery = s3707_esd_recovery,
	.swipe_enable = s3707_swipe_enable,
...
static int __init touch_device_init(void)
{
	...
	if (!(is_ddic_name("sw43410") || is_ddic_name("sw43408a")
				|| is_ddic_name("rm69299a") || is_ddic_name("rm69299b"))) {
		TOUCH_I("%s, s3707 not found.\n", __func__);
		return 0;
	}
	if (is_ddic_name("sw43408a"))
		TOUCH_I("%s, sw43408a s3707 detected\n", __func__);
	else if (is_ddic_name("rm69299a"))
		TOUCH_I("%s, rm69299a Tianma s3706 detected\n", __func__);
	else if (is_ddic_name("rm69299b"))
		TOUCH_I("%s, rm69299b Tovis s3706 detectec\n", __func__);
