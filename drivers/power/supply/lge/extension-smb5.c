...
static void debug_polling(struct smb_charger* chg) {
	...
	int  capping_idc    =  get_effective_result(chg->dc_icl_votable)/1000;
	...
	bool presence_dc  = !wireless_psy ? false : (!power_supply_get_property(wireless_psy,
...
//	wa_avoid_over_voltage_clear(chg);
}
...
/*************************************************************
 * extension for  VOTABLE CALLBACKS
 */

static int extension_dc_icl_vote_callback(struct votable *votable, void *data,
			int icl_ua, const char *client) {
	struct smb_charger *chg = data;
	union power_supply_propval val = { .intval = icl_ua, };

	return smb5_dc_set_prop(chg->dc_psy, POWER_SUPPLY_PROP_CURRENT_MAX, &val);
}
...
int extension_smb5_probe(struct smb_charger *chg)
{
	...
	chg->dc_icl_votable = create_votable("DC_ICL", VOTE_MIN,
					extension_dc_icl_vote_callback,
					chg);
	if (IS_ERR(chg->dc_icl_votable)) {
		rc = PTR_ERR(chg->dc_icl_votable);
		pr_err("can't create_votable DC_ICL, %d", rc);
