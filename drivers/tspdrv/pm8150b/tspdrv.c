...
asmlinkage void _Pm8150bDbgOut(int level, const char *fmt,...)
{
...
asmlinkage static void _Pm8150bDbgOutV(int level, const char *fmt,va_list args)
{
    static char printk_buf[MAX_DEBUG_BUFFER_LENGTH];
    static char prefix[6][4] =
        {" * ", " ! ", " ? ", " I ", " V", " O "};
...
asmlinkage void _Pm8150bDbgOutTemp(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_TEMP <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pm8150bDbgOutV(DBL_TEMP, fmt, args);
        va_end(args);
    }
}

asmlinkage void _Pm8150bDbgOutFatal(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_FATAL <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pm8150bDbgOutV(DBL_FATAL, fmt, args);
        va_end(args);
    }
}

asmlinkage void _Pm8150bDbgOutErr(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_ERROR <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pm8150bDbgOutV(DBL_ERROR, fmt, args);
        va_end(args);
    }
}

asmlinkage void _Pm8150bDbgOutWarn(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_WARNING <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pm8150bDbgOutV(DBL_WARNING, fmt, args);
        va_end(args);
    }
}

asmlinkage void _Pm8150bDbgOutInfo(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_INFO <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pm8150bDbgOutV(DBL_INFO, fmt, args);
        va_end(args);
    }
}

asmlinkage void _Pm8150bDbgOutVerbose(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_VERBOSE <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pm8150bDbgOutV(DBL_VERBOSE, fmt, args);
        va_end(args);
    }
}

asmlinkage void _Pm8150bDbgOutOverkill(const char *fmt,...)
{
    int nDbgLevel = atomic_read(&g_nDebugLevel);
    if (DBL_OVERKILL <= nDbgLevel)
    {
        va_list args;
        va_start(args, fmt);
        _Pm8150bDbgOutV(DBL_OVERKILL, fmt, args);
        va_end(args);
    }
}
...
static ssize_t write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
    ...
    */
    ...
    if (!Pm8150bSaveOutputData(g_cWriteBuffer, count))
    {
        DbgOutErr(("tspdrv: Pm8150bSaveOutputData failed.\n"));
        return -EIO;
    }
