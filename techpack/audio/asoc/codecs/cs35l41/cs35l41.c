...
static const unsigned int ram_table[] = {
    EXTCON_MECHANICAL,
	EXTCON_NONE,
};
#endif
...
#else //always trigger fast_switch function to back default value
	 if (!cs35l41->fast_switch_en && ucontrol->value.integer.value[0]) {
	 	cs35l41->fast_switch_file_idx = 1; //bypass on
		ret = cs35l41_do_fast_switch(cs35l41);
	 } else if(cs35l41->fast_switch_en && !ucontrol->value.integer.value[0]) {
		 cs35l41->fast_switch_file_idx = 0; //bypass off
		 ret = cs35l41_do_fast_switch(cs35l41);
	 }
#endif
...
#ifndef CONFIG_MACH_LGE // Cirrus Original Code
	ret = regmap_bulk_read(cs35l41->regmap, CS35L41_OTP_MEM0, otp_mem,
						CS35L41_OTP_SIZE_WORDS);
#else // Cirrus Modification Code
...
static int cs35l41_pcm_mute(struct snd_soc_dai *dai, int mute)
{
	...
	struct cs35l41_private *cs35l41 = snd_soc_codec_get_drvdata(codec);
	unsigned int vol, vol_ramp, vol_ramp_ms;
				 ...
				 struct snd_soc_dai *dai)
{
...
#ifdef CONFIG_MACH_LGE
	ret = snd_soc_codec_set_sysclk(dai->codec,0, 0, 3072000, 0);
	if (ret < 0)
		dev_dbg(cs35l41->dev, "%s: set pll fail for cs35l41, err:%d\n",__func__,ret);
#endif
	...
	regmap_update_bits(cs35l41->regmap, CS35L41_GLOBAL_CLK_CTRL,
		...
		dev_err(cs35l41->dev, "%s: Unsupported rate\n", __func__);
...
#ifdef CONFIG_MACH_LGE
		dev_dbg(cs35l41->dev, "%s: i2s_mode = %d, right_channel = %d \n",__func__,cs35l41->i2s_mode, cs35l41->pdata.right_channel);
#endif
		if (cs35l41->i2s_mode) {
			regmap_update_bits(cs35l41->regmap,
...
EXPORT_SYMBOL(cs35l41_module_dep);
#endif
				  ...
				  struct cs35l41_private *cs35l41)
{
	...
	if (of_property_read_u32(np, "cirrus,ng-cfg", &val) >= 0)
		pdata->ng_cfg = val;
	else
		pdata->ng_cfg = 0;
				...
				struct cs35l41_platform_data *pdata)
{
	...
	*/
	...
	//Noise Gate CFG update
	if (cs35l41->pdata.ng_cfg > 0x0) {
		regmap_write(cs35l41->regmap, CS35L41_NG_CFG,
				cs35l41->pdata.ng_cfg);
	}
	...
	if (!cs35l41->pdata.right_channel) {
		edev = devm_extcon_dev_allocate(cs35l41->dev, ram_table);
		if (IS_ERR(edev)) {
			dev_err(cs35l41->dev, "failed to allocate extcon device\n");
			return -ENOMEM;
		}

		edev->name = "ram_status";
		ret = devm_extcon_dev_register(cs35l41->dev, edev);
		if (ret < 0) {
			dev_err(cs35l41->dev, "extcon_dev_register() failed: %d\n",
			ret);
			return ret;
		}
		pr_info("%s register extcon device for ram_status\n", __func__);
	}
#endif
