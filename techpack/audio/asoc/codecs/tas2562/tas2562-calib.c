...
static int smartamp_params_ctrl(uint8_t *input, u8 dir, u8 count)
{
	...
	if (!ppacket) {
		pr_err ("[Smartamp:%s] pperror kmalloc failed!", __func__);
		return -ENOMEM;
	}
	...
	if (ret){
		pr_err("[Smartamp:%s] Error copying from user\n", __func__);
		kfree (ppacket);
		return -EFAULT;
	}
	...
	{
		pr_err("[Smartamp:%s] %s Slave 0x%x params failed, paramid mismatch\n", __func__, 
	...
	if (ret)
		pr_err("[Smartamp:%s] Slave 0x%x params failed from afe, ret=%x\n", __func__, 
				dir == TAS_GET_PARAM ? "get" : "set", ppacket->slave_id, ret);
	else
		pr_info("[Smartamp:%s] Algo control returned %d\n", __func__, ret);

	if (dir == TAS_GET_PARAM) {
		ret = copy_to_user(input, ppacket, sizeof(struct tas_dsp_pkt));
		if (ret) {
			pr_err("[Smartamp:%s] Error copying to user after DSP", __func__);
			ret = -EFAULT;
		}
	}
...
{
	int rc;
	pr_err("[Smartamp:%s]",__func__);
	rc = misc_register(&tas_calib_misc);
	if (rc)
		pr_err("[Smartamp:%s] register calib misc failed\n", __func__);
	return rc;
}
