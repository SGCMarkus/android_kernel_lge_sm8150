...
static void cs43130_hph_switch_gpio_set(struct cs43130_private *cs43130,int val);
static void cs43130_hph_power_gpio_set(struct cs43130_private *cs43130,int val);

static int cs43130_clock_control(struct clk *clk,int enable);
static int cs43130_regulator_control(struct regulator *regulator,int enable);

static void cs43130_channel_swap(struct cs43130_private *cs43130);
#endif
...
#ifdef CONFIG_MACH_LGE
	cs43130_hph_switch_gpio_set(cs43130,1);
	cs43130_channel_swap(cs43130);
#endif
	cs43130->clk_req++;
	if (cs43130->clk_req == 2)
		cs43130_pcm_dsd_mix(true, cs43130->regmap);
...
#ifdef CONFIG_MACH_LGE
		cs43130_hph_switch_gpio_set(cs43130,0);
#endif
	}
	mutex_unlock(&cs43130->clk_mutex);
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_err(codec->dev,"cs43130_hpin_event (%x)\n",event);
...
#ifdef CONFIG_MACH_LGE
static const struct snd_kcontrol_new cs43131_dapm_abypass_controls[] = {
...
static const struct snd_soc_dapm_widget analog_hp_widgets[] = {
	...
	SND_SOC_DAPM_MIXER("Cirrus Analog Playback", SND_SOC_NOPM, 0, 0,
			cs43131_dapm_abypass_controls,ARRAY_SIZE(cs43131_dapm_abypass_controls)),
#endif
...
#ifdef CONFIG_MACH_LGE
	{"Cirrus Analog Playback", "Switch", "IN"},
	{"Analog Playback", NULL, "Cirrus Analog Playback"},
#endif	
...
#ifdef CONFIG_MACH_LGE
//set hph_switch_gpio high when it is not bypass mode.
static void cs43130_hph_switch_gpio_set(struct cs43130_private *cs43130,int val)
{
	if ( val >= 1)
		gpio_set_value(cs43130->hph_switch, 1);
	else
		gpio_set_value(cs43130->hph_switch, 0);
	
	pr_info("%s(): cs43130->hph_switch = %d\n", __func__, __gpio_get_value(cs43130->hph_switch));
}

// set high when headset is connected and set low when headset is disconnected
// power_gpio is always set high on ess chip
// temporary power_gpio is set always high
static void cs43130_hph_power_gpio_set(struct cs43130_private *cs43130,int val)
{
	if ( val >= 1)
		gpio_set_value(cs43130->power_gpio, 1);
	else
		gpio_set_value(cs43130->power_gpio, 0);
	
	pr_info("%s(): cs43130->power_gpio = %d\n", __func__, __gpio_get_value(cs43130->power_gpio));
}
...
static int cs43130_clock_control(struct clk *clk,int enable){
	...
	pr_info("%s(): cs43130 clock enable = %d\n", __func__, enable);
...
static int cs43130_regulator_control(struct regulator *regulator,int enable){
	...
	pr_info("%s(): cs43130 regulator enable = %d\n", __func__, enable);
...
static void cs43130_channel_swap(struct cs43130_private *cs43130){
	...
	bool ch_swap = cs43130->channel_swap;
	
	pr_info("%s(): ch_swap = %d\n", __func__, ch_swap);
	
        regmap_update_bits(cs43130->regmap, CS43130_PCM_PATH_CTL_1, 0x44, 0x44);
	...
	if(ch_swap) {
		...
		regmap_update_bits(cs43130->regmap, CS43130_PCM_PATH_CTL_2, 0x02, 0x02);
	}	
}
...
#ifdef CONFIG_MACH_LGE
//Bypass switch gpio setting
	cs43130->hph_switch = of_get_named_gpio(dev->of_node, "dac,hph-sw", 0);
	if (cs43130->hph_switch < 0) {
		pr_err("Looking up %s property in node %s failed %d\n", "dac,hph-sw", dev->of_node->full_name, cs43130->hph_switch);
		return -EINVAL;
	}

	ret = gpio_request(cs43130->hph_switch, "hph_switch");
	if (ret < 0) {
		pr_err("%s(): hph_switch request failed\n", __func__);
		gpio_free(cs43130->hph_switch);
		return -EINVAL;	
	}
	
	ret = gpio_direction_output(cs43130->hph_switch, 1);
	if (ret < 0) {
		pr_err("%s(): hph_switch set failed\n", __func__);
		gpio_free(cs43130->hph_switch);
		return -EINVAL;	
	}
...
//Power LDO gpio setting	
	cs43130->power_gpio = of_get_named_gpio(dev->of_node, "dac,power-gpio", 0);
	if (cs43130->power_gpio < 0) {
		pr_err("Looking up %s property in node %s failed %d\n", "dac,power-gpio", dev->of_node->full_name, cs43130->power_gpio);
		return -EINVAL;
	}
	
	ret = gpio_request(cs43130->power_gpio, "hph_power");
	if (ret < 0) {
		pr_err("%s(): hph_power request failed\n", __func__);
		gpio_free(cs43130->power_gpio);
		return -EINVAL;	
	}
	
	ret = gpio_direction_output(cs43130->power_gpio, 1);
	if (ret < 0) {
		pr_err("%s(): hph_power set failed\n", __func__);
		gpio_free(cs43130->power_gpio);
		return -EINVAL;	
	}
	
	cs43130_hph_power_gpio_set(cs43130, 1); //temporary power_gpio is set always high
//temporary set enable clock when driver is probed.
//need to change enable/disable clock
	cs43130->ln_bb_clk3 = devm_clk_get(dev, "ln_bb_clk3");
	if (IS_ERR(cs43130->ln_bb_clk3)) {
		ret = PTR_ERR(cs43130->ln_bb_clk3);
		pr_err("%s(): ln_bb_clk3 clock get fail %d\n", __func__,ret);
		return -EINVAL;		
	}
	
	ret =  cs43130_clock_control(cs43130->ln_bb_clk3,1); //enable the clk
	if (ret < 0) {
		pr_err("%s(): ln_bb_clk3 enable fail %d\n", __func__,ret);
		return -EINVAL;	
	}

//temporary set Amp LDO regulator when driver is probed.
//need to change enable/disable regulator
//Amp LDO Regulator
	if (of_property_read_bool(dev->of_node, "dac,vdd-supply")){
		cs43130->dac_vdd_regulator = devm_regulator_get(dev,"dac,vdd");
		if(IS_ERR(cs43130->dac_vdd_regulator)){
			ret = PTR_ERR(cs43130->dac_vdd_regulator);
			pr_err("%s(): dac_vdd_regulator get fail %d\n", __func__,ret);
			return -EINVAL;
		}
		
		ret = cs43130_regulator_control(cs43130->dac_vdd_regulator,1);
			if (ret < 0) {
			pr_err("%s(): dac_vdd_regulator enable fail %d\n", __func__,ret);
			return -EINVAL;	
		}
	}
//temporary set Comparator regulator when driver is probed.
//need to change enable/disable regulator
//Comparator LDO Regulator
	if (of_property_read_bool(dev->of_node, "dac,comparator-supply")){
		cs43130->dac_comparator_regulator = devm_regulator_get(dev,"dac,comparator");
		if(IS_ERR(cs43130->dac_comparator_regulator)){
			ret = PTR_ERR(cs43130->dac_comparator_regulator);
			pr_err("%s(): dac_comparator_regulator get fail %d\n", __func__,ret);
			return -EINVAL;
		}
	
		ret = cs43130_regulator_control(cs43130->dac_comparator_regulator,1);
			if (ret < 0) {
			pr_err("%s(): dac_vdd_regulator enable fail %d\n", __func__,ret);
			return -EINVAL;	
		}
	}
//PCM Channel Swap for Rev0
	if (of_property_read_bool(dev->of_node, "dac,channel_swap")){
		pr_info("%s : dac,channel_swap enable\n",__func__);
		cs43130->channel_swap = true;
	} else {
		pr_info("%s : dac,channel_swap disable\n",__func__);
		cs43130->channel_swap = false;		
	}
#endif
	return 0;
}
			     ...
			     const struct i2c_device_id *id)
{
	...
	dev_err(&client->dev, "cs43130_i2c_probe enter\n");
...
//temporary set enable clock when driver is probed.
//need to change enable/disable clock
	cs43130_clock_control(cs43130->ln_bb_clk3,0); //disable the clk
	
//temporary set Amp LDO regulator when driver is probed.
//need to change enable/disable regulator
//Amp LDO Regulator
	if (of_property_read_bool(dev->of_node, "dac,vdd-supply")){
		ret = cs43130_regulator_control(cs43130->dac_vdd_regulator,0);
			if (ret < 0) {
			pr_err("%s(): dac_vdd_regulator disable fail %d\n", __func__,ret);
			return -EINVAL;	
		}
	}
...
//Comparator LDO Regulator
	if (of_property_read_bool(dev->of_node, "dac,comparator-supply")){
		ret = cs43130_regulator_control(cs43130->dac_comparator_regulator,0);
			if (ret < 0) {
			pr_err("%s(): dac_vdd_regulator disable fail %d\n", __func__,ret);
			return -EINVAL;	
		}
...
EXPORT_SYMBOL(cs43130_module_dep);
#endif
