...
static const char *hooks_stats_strings[HOOKS_STATS_COUNT] =
{
    ...
    [HOOKS_STATS_ORPHANED] = "packet orphaned",
};
...
struct KernelSpdFields
{
    ...
    const char *in_name;
    const char *out_name;
    const char *unprotected_name;

    int in_interface_index;
        ...
        const struct net_device *out)
{
    ...
    if (in != NULL)
    {
        ...
        spdfields->in_interface_index = in->ifindex;
    }

    if (out != NULL)
    {
        spdfields->out_name = out->name;
        spdfields->out_interface_index = out->ifindex;
    }
    ...
    if (in_protected && !out_protected)
    {
        ...
        spdfields->unprotected_name = spdfields->out_name;
    }
    else if (!in_protected && out_protected)
    {
        ...
        spdfields->unprotected_name = spdfields->in_name;
    }
    else if (!in_protected && !out_protected)
    {
...
static inline void
kernelspd_skb_set_interfaces(
        struct sk_buff *skb,
        const struct KernelSpdFields *fields)
{
    uint32_t *interfaces = (uint32_t *) skb->cb;

    interfaces[0] = fields->in_interface_index;
    interfaces[1] = fields->out_interface_index;
}
        ...
        uint32_t *out_interface_index_p)
{
    ...
    *in_interface_index_p = interfaces[0];
    *out_interface_index_p = interfaces[1];
}
EXPORT_SYMBOL(kernelspd_skb_get_interfaces);
        ...
        int result)
{
    ...
    case KERNEL_SPD_PROTECT:
        kernelspd_skb_set_interfaces(skb, spdfields);
        ...
        uint32_t fragment_id)
{
    ...
    fill_spd_fields(spd_net, &spdfields, skb, hook, INVALID_UID, in, out);
...
EXPORT_SYMBOL(kernelspd_mark_reinject);
